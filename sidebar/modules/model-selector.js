...
export class ModelSelector {
  ...
  async init() {
    ...
    logger.info('Model selector initialization completed');
  }

  // Load available models from configuration
  async loadModels() {
    try {
      logger.info('Loading available models from configuration');
  ...
  renderModelOptions() {
    ...
    logger.debug(`Rendered ${this.models.length} model options`);
  }
  ...
  notifyModelChange() {
    ...
    logger.debug('Model change event dispatched');
  }
  ...
  async refresh() {
    logger.info('Refreshing model selector');
    await this.loadModels();
  }
